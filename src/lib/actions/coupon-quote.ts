...
function quoteFrom(value: unknown): CouponQuote | null {
  ...
  return { couponDiscountCents: centsValues[0]!, merchandiseSubtotalCents: centsValues[1]!, promotionDiscountCents: centsValues[2]!, shippingCents: centsValues[3]!, defaultSource: row.default_source as CouponQuoteSource, selectedSource: row.selected_source as CouponQuoteSource, winningSource: row.winning_source as CouponQuoteSource };
}
...
export async function quoteCouponCheckout(items: CheckoutCartItem[], couponCode: string, selectedSource?: CouponQuoteSource): Promise<{ error: string; success: false } | { quote: CouponQuote; success: true }> {
  ...
  const { data, error } = await supabaseAdmin.rpc("quote_coupon_checkout", { p_product_ids: parsed.data.map((item) => item.product.id), p_coupon_code: code, p_identity_key_version: "v1", p_identity_fingerprint: fingerprint, p_shipping_cents: 0, p_selected_source: selectedSource ?? null });
